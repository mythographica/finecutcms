...
(function($) {
	...
	$.fn.elBorderSelect = function(o) {
		...
		var style = $('<div />').elSelect({
			tpl       : '<div style="border-bottom:4px %val #000;width:90%;margin:7px 0"> </div>',
			tpls      : { '' : '%label'},
	...
	.bind('keydown', function(e) {
		...
		if (e.ctrlKey && e.keyCode == 83) {
			try{
				e.stopPropagation();
				e.preventDefault();
				if( self.options.megaSave !== undefined ){
					self.options.megaSave();
				}
				// alert( obj );
				// alert(window.megaSaveFunction)
				// alert(document.body.megaSaveFunction)
			}catch(e){ alert(e); }
		}
...
elRTE.prototype.options   = {
	...
	panels      : {
		...
		cpspvinni  : ['pastetext', 'pasteformattext', 'removeformat', 'docstructure'],
		undoredo   : ['undo', 'redo'],
	...
	toolbars    : {
		...
		vinni    : ['cpspvinni', 'undoredo', 'alignment', 'lists', 'links', 'images', 'fullscreen', 'format', 'style', 'colors', 'indent',  'tables' ]
...
elRTE.prototype.ui.prototype.buttons.anchor = function(rte, name) {
	...
	this.command = function() {
		...
		this.input.val($(this.anchor).addClass('elrte-anchor').attr('name')).css('width',150);
...
(function($) {
	...
	elRTE.prototype.ui.prototype.buttons.flash = function(rte, name) {
		...
		this.src = {
			url    : $('<input type="text" name="url" />').css('width', '90%'),
			...
			width  : $('<input type="text" style = "width: 30px"/>').attr('size', 5).css('text-align', 'right'),
			height : $('<input type="text" style = "width: 30px"/>').attr('size', 5).css('text-align', 'right'),
			wmode  : $('<select style = "width: 100px"/>')
						.append($('<option />').val('').text(this.rte.i18n('Not set', 'dialogs')))
						.append($('<option />').val('transparent').text(this.rte.i18n('Transparent'))),
			align  : $('<select style = "width: 100px"/>')
						.append($('<option />').val('').text(this.rte.i18n('Not set', 'dialogs')))
		...
		this.command = function() {
			...
			var fs = $('<fieldset style = "width: 500px"/>').append($('<legend />').text(this.rte.i18n('Preview')))
			d.append(fs, 'main');
...
elRTE.prototype.ui.prototype.buttons.fullscreen = function(rte, name) {
	...
	this.command = function() {
		...
		if (f) {
			...
			wz.height(w.height() - this.delta).css('width', '90%');
...
elRTE.prototype.ui.prototype.buttons.horizontalrule = function(rte, name) {
	...
	this.src = {
		...
		bg      : $('<div />'),
		border  : $('<div />'),
		'class' : $('<input type="text" />').css('width', '90%'),
		style   : $('<input type="text" />').css('width', '90%')
	}
	...
	this.command = function() {
		...
		this.hr = n.nodeName == 'HR' ? $(n) : $(rte.doc.createElement('hr')).css({width : '90%', height : '1px'});
...
elRTE.prototype.ui.prototype.buttons.image = function(rte, name) {
	...
	this.init = function() {
		...
		this.src = {
			main : {
				src    : $('<input type="text" />').css('width', '90%').change(preview),
				title  : $('<input type="text" />').css('width', '90%'),
				alt    : $('<input type="text" />').css('width', '90%'),
				width  : $('<input type="text" />').attr('size', 5).css('text-align', 'right').change(size).css('width', '10%'),
				height : $('<input type="text" />').attr('size', 5).css('text-align', 'right').change(size).css('width', '10%'),
		...
		$.each(['id', 'class', 'style', 'longdesc'], function(i, name) {
			self.src.adv[name] = $('<input type="text" style="width:90%" />');
		});
		...
		$.each(
			...
			function() {
				self.src.events[this] = $('<input type="text"  style="width:90%"/>');
		});
	}
	...
	this.command = function() {
		...
		dialog.append($('<fieldset style = "display: none"><legend>'+this.rte.i18n('Preview')+'</legend></fieldset>').append(this.preview), 'main');
	...
	function init() {
		...
		$.each(self.src, function() {
			for (var n in this) {
				...
				if (t == 'INPUT' && n != 'bgimg') {
					this[n].css(this[n].attr('size') ? {'text-align' : 'right'} : {width : '90%'});
		...
		$.each(
			['onblur', 'onfocus', 'onclick', 'ondblclick', 'onmousedown', 'onmouseup', 'onmouseover', 'onmouseout', 'onmouseleave', 'onkeydown', 'onkeypress', 'onkeyup'],
			function() {
				self.src.events[this] = $('<input type="text" />').css('width', '90%');
		});
