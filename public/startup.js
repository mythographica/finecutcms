( function( undefined ) { $(document).ready(function() {
	...
	try{
		...
		var ServerPaths = {
			  pages		: '/engine/tree'
			, templates	: '/engine/template-action'
			, api		: '/engine/api'
		};
		...
		var helpers = {
			...
			, setValues : function( obj ){
				try{
					...
					}else if( typeof obj.blocks === 'string' ){
						obj.blocks = JSON.parse( obj.blocks );
					}
					// else: already an object (array), use as-is
		...
		$('#elfinder').elfinder({
			  url: '/engine/elfinder'  // connector URL (REQUIRED)
			, height: '530px'
		...
		bindAceKeys.variator = function( bond, objt ){
			var moveCursor = true;
			var cursorPosition = bond.getCursorPosition();
			if(typeof(objt) == 'string'){
				cursorPosition.column += ( 2 + objt.length );
				bond.insert('<' + objt + '></' + objt + '>');
			}else{
				if($.isArray(objt)){
					cursorPosition.column += objt[0].length;
					var ins = objt.join('');
					bond.insert(ins);
				}
				if($.isFunction(objt)){
		...
		$('#pages-editor-container').on('click', function(evt){
			return false;
		});

		$('#settings-editor-text-container').on('click', function(){
			return false;
		});
		...
		var fillTheLink = function( bond ){
			var val = prompt('Please Fill Link!', 'http://');
			if(val){
				var str = '<a href = "'+val+'"';
				confirm(' New Tab (OK) or This tab (Cancel)?') && ( str += ' target = "_blank" ');
				str += '>';
				bindAceKeys.variator( bond, [str, '</a>'] );
			}
		};
		
		$( '#pages-editor-buttons-Link' ).click( function(){
			fillTheLink( ACEditors.pages );
			ACEditors.pages.focus();
		} );

		$( '#pages-editor-buttons-Image' ).click( function(){
			var val = prompt('Please Fill SRC!', 'http://');
			if(val){
...
+ default ACE Editor preset\n\n';
		...
		$('#templ-source-container').on('click', function(){
			return false;
		});
		...
		var saveSettingsAction = function(){ // used for keydown Ctrl + S reaction
			// $('#settings-editor-text').val();
			var val = ACEditors.settings.getValue();
			var settings = JSON.stringify( { val: val } );
			$.ajax( {
				  type: "POST"
				, async: true
				...
				, success: function( data ){
					...
					ACEditors.settings.setValue( data, -1 );
		...
		window.setTimeout(function(){
			...
			var resizer = function(){
				...
				if( wh > starter + 7 ){
					...
					$('#elfinder').find('div.el-finder-nav, div.el-finder-cwd').height( 530 );
				}
